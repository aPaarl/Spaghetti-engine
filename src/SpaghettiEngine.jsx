...
function makeTemplate(kind) {
  ...
  if (kind === "nexus") {
    // A generic water-energy-food-ecosystems nexus in five modules of eight
    // concepts. Each module is a coherent sub-model with its own internal
    // feedback; the modules are coupled through a smaller set of interface
    // relationships (water for crops and hydropower, energy for fertiliser,
    // land for bioenergy, emissions for climate, policy levers from the
    // governance module). The three policy instruments have no incoming
    // relationships, so they act as levers that scenarios switch on; without
    // them the baseline drifts towards a degraded state. Illustrative only:
    // the structure and weights show how the tool handles a modular nexus,
    // not a calibrated model of any real system. Tuned so that the baseline
    // and every scenario converge under the default settings.
    const c = [
      // --- Water --------------------------------------------------------------
      ["W_AVL", "Freshwater availability", "Water", 0, "Renewable surface and groundwater that can be used each year, relative to the long-term average."],
      ["W_GW", "Groundwater levels", "Water", 0, "Depth of the groundwater table; higher values mean more water stored in aquifers."],
      ["W_DEM", "Total water demand", "Water", 0.25, "Water withdrawn for all uses together: households, industry, energy, and agriculture."],
      ["W_IRR", "Irrigation water use", "Water", 0, "Volume of water applied to crops through irrigation."],
      ["W_QUAL", "Water quality", "Water", 0, "Chemical and ecological quality of rivers, lakes, and groundwater."],
      ["W_DRO", "Drought frequency", "Water", 0, "How often and how severely dry spells reduce water supply."],
      ["W_TRT", "Wastewater treatment capacity", "Water", 0, "Share of wastewater collected and treated before it is returned to the environment."],
      ["W_PRC", "Water price", "Water", 0, "Price paid per unit of water by users, including abstraction charges."],
      // --- Energy -------------------------------------------------------------
      ["E_DEM", "Energy demand", "Energy", 0.5, "Total final energy consumption across sectors."],
      ["E_REN", "Renewable energy capacity", "Energy", 0.25, "Installed capacity of wind, solar, and other renewable generation."],
      ["E_FOS", "Fossil fuel use", "Energy", 0.5, "Coal, oil, and gas burned for power, heat, and transport."],
      ["E_PRC", "Energy price", "Energy", 0, "Average price of electricity and fuels for end users."],
      ["E_BIO", "Bioenergy crop production", "Energy", 0, "Crops and residues grown or collected to produce biofuels and biomass energy."],
      ["E_HYD", "Hydropower output", "Energy", 0, "Electricity generated by hydropower plants."],
      ["E_EFF", "Energy efficiency", "Energy", 0, "Useful output obtained per unit of energy used (insulation, efficient equipment)."],
      ["E_GRD", "Grid flexibility and storage", "Energy", 0, "Ability of the power system to balance variable renewable supply (storage, demand response, interconnection)."],
      // --- Food & Agriculture -----------------------------------------------------
      ["F_PRD", "Agricultural production", "Food", 0.25, "Total output of crops and livestock products."],
      ["F_YLD", "Crop yields", "Food", 0, "Harvest per hectare for the main crops."],
      ["F_FRT", "Fertiliser use", "Food", 0.5, "Nitrogen and phosphorus fertiliser applied per hectare."],
      ["F_LND", "Agricultural land area", "Food", 0, "Area used for crops and grazing."],
      ["F_LIV", "Livestock numbers", "Food", 0.25, "Number of farm animals, in livestock units."],
      ["F_SEC", "Food security", "Food", 0, "Reliable access to sufficient, affordable, and nutritious food for the population."],
      ["F_PRC", "Food prices", "Food", 0, "Consumer prices of staple foods."],
      ["F_WST", "Food loss and waste", "Food", 0, "Share of food produced that is lost in the chain or thrown away."],
      // --- Ecosystems & Climate ------------------------------------------------------
      ["C_GHG", "Greenhouse gas emissions", "Ecosystems & Climate", 0.5, "Annual emissions of CO2, methane, and nitrous oxide from all sources."],
      ["C_TMP", "Climate warming", "Ecosystems & Climate", 0.25, "Rise in average temperature and the associated climate extremes."],
      ["C_BIO", "Biodiversity", "Ecosystems & Climate", 0, "Abundance and variety of species and habitats."],
      ["C_SOL", "Soil health", "Ecosystems & Climate", 0, "Soil organic matter, structure, and biological activity."],
      ["C_FOR", "Natural habitat area", "Ecosystems & Climate", 0, "Area of forest, wetland, and other natural or semi-natural habitat."],
      ["C_NUT", "Nutrient pollution", "Ecosystems & Climate", 0.25, "Nitrogen and phosphorus losses to water and air."],
      ["C_POL", "Pollination services", "Ecosystems & Climate", 0, "Contribution of wild pollinators to crop production."],
      ["C_SEQ", "Carbon sequestration", "Ecosystems & Climate", 0, "Carbon taken up and stored by vegetation and soils."],
      // --- Governance & Society -------------------------------------------------------
      ["G_POP", "Population and economic growth", "Governance & Society", 0.5, "Growth in population and economic activity, driving demand for resources."],
      ["G_INC", "Household income", "Governance & Society", 0.25, "Average disposable income of households."],
      ["G_WPL", "Water allocation policy", "Governance & Society", 0, "Rules and permits that cap and allocate water use between sectors."],
      ["G_CPL", "Climate and energy policy", "Governance & Society", 0, "Carbon pricing, renewable energy targets, and related regulation. A policy lever: set by the scenario, not by other concepts."],
      ["G_APL", "Agri-environmental measures", "Governance & Society", 0, "Subsidies and regulation that reward sustainable farming practices. A policy lever: set by the scenario, not by other concepts."],
      ["G_AWR", "Public environmental awareness", "Governance & Society", 0, "Public concern about environmental problems and support for action."],
      ["G_RND", "Research and innovation", "Governance & Society", 0, "Investment in research, development, and uptake of new technology."],
      ["G_CNF", "Resource-use conflicts", "Governance & Society", 0, "Disputes between users or sectors over water, land, and energy."],
    ];
    const e = [
      // Water, internal
      ["W_DEM", "W_AVL", -0.75], ["W_IRR", "W_DEM", 0.75], ["W_IRR", "W_GW", -0.5], ["W_GW", "W_AVL", 0.5],
      ["W_DRO", "W_AVL", -0.75], ["W_DRO", "W_IRR", 0.5], ["W_TRT", "W_QUAL", 0.75], ["W_QUAL", "W_AVL", 0.25],
      ["W_AVL", "W_PRC", -0.5], ["W_PRC", "W_DEM", -0.5], ["W_PRC", "W_IRR", -0.5],
      // Energy, internal
      ["E_DEM", "E_FOS", 0.75], ["E_DEM", "E_PRC", 0.5], ["E_REN", "E_FOS", -0.25], ["E_PRC", "E_DEM", -0.5],
      ["E_PRC", "E_REN", 0.5], ["E_EFF", "E_DEM", -0.75], ["E_HYD", "E_FOS", -0.25], ["E_BIO", "E_FOS", -0.25],
      ["E_GRD", "E_REN", 0.5], ["E_REN", "E_GRD", 0.25], ["E_REN", "E_PRC", -0.25],
      // Food, internal
      ["F_FRT", "F_YLD", 0.5], ["F_YLD", "F_PRD", 0.75], ["F_LND", "F_PRD", 0.5], ["F_LIV", "F_PRD", 0.5],
      ["F_PRD", "F_PRC", -0.5], ["F_PRD", "F_SEC", 0.75], ["F_PRC", "F_SEC", -0.75], ["F_WST", "F_SEC", -0.5],
      ["F_PRC", "F_LND", 0.5], ["F_PRC", "F_FRT", 0.25], ["F_LIV", "F_LND", 0.5],
      // Ecosystems & Climate, internal
      ["C_GHG", "C_TMP", 0.75], ["C_TMP", "C_BIO", -0.5], ["C_FOR", "C_BIO", 0.75], ["C_FOR", "C_SEQ", 0.75],
      ["C_SEQ", "C_GHG", -0.5], ["C_NUT", "C_BIO", -0.75], ["C_BIO", "C_POL", 0.75], ["C_SOL", "C_SEQ", 0.5],
      ["C_BIO", "C_SOL", 0.25], ["C_TMP", "C_FOR", -0.25],
      // Governance & Society, internal
      ["G_POP", "G_INC", 0.5], ["G_INC", "G_AWR", 0.25], ["G_AWR", "G_WPL", 0.25], ["G_RND", "G_INC", 0.25], ["G_CNF", "G_WPL", 0.5], ["G_CPL", "G_RND", 0.5],
      ["G_INC", "G_CNF", -0.25],
      // Interface: water and food
      ["W_AVL", "F_YLD", 0.5], ["W_IRR", "F_YLD", 0.5], ["F_LND", "W_IRR", 0.5],
      // Interface: water and energy
      ["W_AVL", "E_HYD", 0.75], ["E_PRC", "W_TRT", -0.25], ["W_DEM", "E_DEM", 0.25], ["E_FOS", "W_DEM", 0.25],
      // Interface: energy and food
      ["E_BIO", "F_PRC", 0.5], ["E_PRC", "F_FRT", -0.5], ["E_PRC", "F_PRC", 0.5],
      // Interface: energy, food, and ecosystems & climate
      ["E_FOS", "C_GHG", 1], ["E_BIO", "C_FOR", -0.25], ["F_LIV", "C_GHG", 0.5], ["F_LND", "C_FOR", -0.75],
      ["F_FRT", "C_NUT", 0.75], ["F_LIV", "C_NUT", 0.5], ["F_FRT", "C_SOL", -0.25],
      ["C_POL", "F_YLD", 0.5], ["C_SOL", "F_YLD", 0.5], ["C_TMP", "F_YLD", -0.5],
      // Interface: ecosystems & climate and water
      ["C_TMP", "W_DRO", 0.75], ["C_NUT", "W_QUAL", -0.75],
      // Interface: governance and the other modules
      ["G_POP", "E_DEM", 0.5], ["G_INC", "F_WST", 0.25], ["G_INC", "F_SEC", 0.5],
      ["G_WPL", "W_IRR", -0.5], ["G_WPL", "W_PRC", 0.5],
      ["G_CPL", "E_REN", 0.75], ["G_CPL", "E_FOS", -0.5], ["G_CPL", "E_PRC", 0.25],
      ["G_APL", "F_FRT", -0.5], ["G_APL", "C_FOR", 0.25], ["G_APL", "C_SOL", 0.25],
      ["G_RND", "E_EFF", 0.5], ["G_RND", "F_YLD", 0.25], ["G_RND", "W_TRT", 0.25],
      ["W_AVL", "G_CNF", -0.75], ["F_SEC", "G_CNF", -0.5], ["F_PRC", "G_INC", -0.25], ["E_PRC", "G_INC", -0.25],
    ];
    const concepts = layoutByModule(c.map(([id, name, category, initialValue, description]) => ({
      id, name, category, description, initialValue, currentValue: initialValue, position: { x: 0, y: 0 },
    })));
    const edges = e.map(([source, target, weight]) => ({ id: uid("e"), source, target, weight, description: "" }));
    // Three ready-made scenarios to compare on the Scenarios & Simulation
    // tab: one lever whose effect spreads through the whole nexus, one whose
    // effect stays mostly in the land and ecosystem modules, and a bundle.
    const scenarios = [
      { ...BASELINE_SCENARIO },
      { id: "s_carbon", name: "Climate and energy policy push", type: "policy_bundle", description: "Climate and energy policy held at full strength for the whole run.", initialOverrides: { G_CPL: 1 }, lockedConcepts: { G_CPL: 1 }, weightOverrides: {} },
      { id: "s_agri", name: "Agri-environmental package", type: "intervention", description: "Agri-environmental measures held at full strength.", initialOverrides: { G_APL: 1 }, lockedConcepts: { G_APL: 1 }, weightOverrides: {} },
      { id: "s_bundle", name: "Integrated nexus policy package", type: "policy_bundle", description: "Climate and energy policy, agri-environmental measures, and water allocation policy all held at full strength.", initialOverrides: { G_CPL: 1, G_APL: 1, G_WPL: 1 }, lockedConcepts: { G_CPL: 1, G_APL: 1, G_WPL: 1 }, weightOverrides: {} },
    ];
...
function readAutosave() {
  try {
    ...
    // Cleaned like an import: a damaged saved copy must never be able to
    // break the app on every visit.
    const clean = sanitizeModel(data);
    return clean ? { ...clean, savedAt: data.savedAt } : null;
  } catch {
    return null;
  }
}
...
// Every model that enters the app from outside (an imported file, or the
// copy autosaved in this browser) passes through here first. Files edited by
// hand, produced by other tools, or saved by older versions can carry
// missing names, text where numbers belong, duplicate ids, or relationships
// pointing at concepts that no longer exist; any of those could break a tab
// or, restored on every visit, the whole app. What can be repaired is
// repaired and listed in `fixes`; what cannot (a relationship to a concept
// that does not exist) is dropped and listed too. Returns null when the
// input is not a model at all.
function sanitizeModel(data) {
  if (!data || typeof data !== "object" || !Array.isArray(data.concepts)) return null;
  const fixes = [];
  const note = (n, one, many) => { if (n) fixes.push(n === 1 ? one : many.replace("{n}", n)); };
  const num = (v) => (typeof v === "number" ? v : typeof v === "string" && v.trim() !== "" ? Number(v.replace(",", ".")) : NaN);
  const clamp1 = (v) => Math.max(-1, Math.min(1, v));

  // concepts ---------------------------------------------------------------
  const usedIds = new Set();
  let unnamed = 0, badStart = 0, duplicates = 0, noPosition = 0;
  const concepts = [];
  data.concepts.forEach((raw, i) => {
    if (!raw || typeof raw !== "object") return;
    let id = raw.id === undefined || raw.id === null || raw.id === "" ? uid("c") : String(raw.id);
    // A repeated id gets a fresh one; relationships that use the id keep
    // pointing at its first occurrence.
    if (usedIds.has(id)) { duplicates += 1; id = uid("c"); }
    usedIds.add(id);
    let name = raw.name === undefined || raw.name === null ? "" : String(raw.name).trim();
    if (!name) { unnamed += 1; name = `Unnamed concept ${i + 1}`; }
    let start = num(raw.initialValue);
    if (raw.initialValue !== undefined && !Number.isFinite(start)) badStart += 1;
    start = Number.isFinite(start) ? clamp1(start) : 0;
    const cur = num(raw.currentValue);
    const px = num(raw.position?.x), py = num(raw.position?.y);
    let position = { x: px, y: py };
    if (!Number.isFinite(px) || !Number.isFinite(py)) { noPosition += 1; position = { x: (i % 8) * 200, y: Math.floor(i / 8) * 140 }; }
    concepts.push({
      ...raw, id, name,
      category: raw.category === undefined || raw.category === null ? "" : String(raw.category),
      description: raw.description === undefined || raw.description === null ? "" : String(raw.description),
      initialValue: start, currentValue: Number.isFinite(cur) ? clamp1(cur) : start, position,
    });
  });
  note(unnamed, "1 concept had no name and was called \"Unnamed concept\".", "{n} concepts had no name and were called \"Unnamed concept\".");
  note(badStart, "1 starting value was not a number and was set to 0.", "{n} starting values were not numbers and were set to 0.");
  note(duplicates, "1 concept had the same id as another and was given a new one.", "{n} concepts had the same id as another and were given new ones.");
  note(noPosition, "1 concept had no position on the map and was placed automatically.", "{n} concepts had no position on the map and were placed automatically.");

  // relationships -----------------------------------------------------------
  const conceptIds = new Set(concepts.map((c) => c.id));
  const edgeIds = new Set(), pairs = new Set();
  let dangling = 0, badWeight = 0, dupPairs = 0;
  const edges = [];
  (Array.isArray(data.edges) ? data.edges : []).forEach((raw) => {
    if (!raw || typeof raw !== "object") return;
    const source = String(raw.source ?? ""), target = String(raw.target ?? "");
    if (!conceptIds.has(source) || !conceptIds.has(target)) { dangling += 1; return; }
    const pair = `${source}>${target}`;
    if (pairs.has(pair)) { dupPairs += 1; return; }
    pairs.add(pair);
    let w = num(raw.weight);
    if (!Number.isFinite(w)) { badWeight += 1; w = 0; }
    let id = raw.id === undefined || raw.id === null || raw.id === "" ? uid("e") : String(raw.id);
    if (edgeIds.has(id)) id = uid("e");
    edgeIds.add(id);
    edges.push({ ...raw, id, source, target, weight: clamp1(w), description: raw.description === undefined || raw.description === null ? "" : String(raw.description) });
  });
  note(dangling, "1 relationship pointed to a concept that does not exist and was left out.", "{n} relationships pointed to concepts that do not exist and were left out.");
  note(dupPairs, "1 relationship repeated another between the same two concepts and was left out.", "{n} relationships repeated others between the same two concepts and were left out.");
  note(badWeight, "1 relationship had no valid weight and was set to 0 (see the Model check).", "{n} relationships had no valid weight and were set to 0 (see the Model check).");

  // scenarios: keep only overrides that still refer to something -------------
  const keepKeys = (obj, valid) => Object.fromEntries(Object.entries(obj && typeof obj === "object" ? obj : {})
    .filter(([k, v]) => valid.has(k) && Number.isFinite(num(v))).map(([k, v]) => [k, clamp1(num(v))]));
  const finalEdgeIds = new Set(edges.map((e) => e.id));
  let scenarios = (Array.isArray(data.scenarios) ? data.scenarios : []).filter((s) => s && typeof s === "object" && s.id).map((s) => ({
    ...s, id: String(s.id), name: String(s.name ?? "").trim() || "Unnamed scenario", type: s.type || "intervention", description: String(s.description ?? ""),
    initialOverrides: keepKeys(s.initialOverrides, conceptIds), lockedConcepts: keepKeys(s.lockedConcepts, conceptIds), weightOverrides: keepKeys(s.weightOverrides, finalEdgeIds),
  }));
  if (!scenarios.some((s) => s.id === "baseline")) scenarios = [{ ...BASELINE_SCENARIO }, ...scenarios];
  ...
  return { concepts, edges, scenarios, settings, fixes };
}
...
function InfluenceEdge({
  ...
  // thickness grows with how many there are, and its label gives the count
  // and net weight instead of a single weight.
  const agg = data?.aggregate || null;
  const color = agg ? aggregateColor(agg) : w >= 0 ? "#16a34a" : "#ea580c";
  ...
  const strokeWidth = agg
    ? 2.5 + Math.min(agg.count, 8) * 0.9 + (selected ? 1.5 : 0)
    : 2.25 + Math.abs(w) * 5.5 + (selected ? 1.5 : 0);
...
// ============================================================================
// Foldable modules (Network tab)
// ============================================================================
// A module can be folded into a single block and unfolded again, so each
// module works as a sub-model of the nexus: the map can show every concept
// (full model), only the modules and the links between them (module
// overview), or one module opened up inside the others (module view).
// Folding changes only how the map is drawn; the model and every analysis
// are untouched.
const MODULE_NODE_W = 220, MODULE_NODE_H = 104;
const MODULE_PREFIX = "module:";
const moduleNodeId = (key) => `${MODULE_PREFIX}${key}`;
const isModuleNodeId = (id) => typeof id === "string" && id.startsWith(MODULE_PREFIX);
const isFrameNodeId = (id) => typeof id === "string" && id.startsWith("frame:");

// Folds the (already scoped) network for display. Concepts of a folded
// module are replaced by one module block, placed at the centre of those
// concepts so unfolding puts everything back where it was. Relationships
// inside a folded module disappear into it. Every other relationship that
// touches a folded module is bundled with the others between the same two
// endpoints into one aggregate link, which keeps count, sign mix, and net
// weight. Relationships between two unfolded concepts pass through untouched
// and stay fully editable.
function foldNetwork(concepts, edges, collapsedSet) {
  const moduleOf = new Map(concepts.map((c) => [c.id, moduleKey(c.category)]));
  const rep = (id) => (collapsedSet.has(moduleOf.get(id)) ? moduleNodeId(moduleOf.get(id)) : id);
  const members = new Map();
  concepts.forEach((c) => {
    const k = moduleOf.get(c.id);
    if (!members.has(k)) members.set(k, []);
    members.get(k).push(c);
  });
  const stats = new Map([...members.keys()].map((k) => [k, { internal: 0, out: 0, in: 0, partners: new Map() }]));
  const plainEdges = [];
  const groups = new Map();
  edges.forEach((e) => {
    const ms = moduleOf.get(e.source), mt = moduleOf.get(e.target);
    if (ms === undefined || mt === undefined) return;
    if (ms === mt) stats.get(ms).internal += 1;
    else {
      stats.get(ms).out += 1; stats.get(mt).in += 1;
      const pOut = stats.get(ms).partners, pIn = stats.get(mt).partners;
      pOut.set(mt, { out: (pOut.get(mt)?.out || 0) + 1, in: pOut.get(mt)?.in || 0 });
      pIn.set(ms, { out: pIn.get(ms)?.out || 0, in: (pIn.get(ms)?.in || 0) + 1 });
    }
    const rs = rep(e.source), rt = rep(e.target);
    if (rs === e.source && rt === e.target) { plainEdges.push(e); return; }
    if (rs === rt) return; // inside one folded module
    const key = `${rs}>${rt}`;
    if (!groups.has(key)) groups.set(key, { id: `agg:${key}`, source: rs, target: rt, edgeIds: [], count: 0, positive: 0, negative: 0, sum: 0 });
    const g = groups.get(key);
    g.edgeIds.push(e.id); g.count += 1; g.sum += e.weight;
    if (e.weight >= 0) g.positive += 1; else g.negative += 1;
  });
  const modules = listModules(concepts);
  const moduleNodes = modules.filter((m) => collapsedSet.has(m.key)).map((m) => {
    const ms = members.get(m.key) || [];
    const cx = ms.reduce((s, c) => s + (c.position?.x ?? 0), 0) / Math.max(1, ms.length) + 80;
    const cy = ms.reduce((s, c) => s + (c.position?.y ?? 0), 0) / Math.max(1, ms.length) + 35;
    return {
      key: m.key, id: moduleNodeId(m.key), label: m.label, categoryValue: m.categoryValue,
      count: ms.length, memberIds: ms.map((c) => c.id), ...stats.get(m.key),
      position: { x: cx - MODULE_NODE_W / 2, y: cy - MODULE_NODE_H / 2 },
    };
  });
  return {
    visibleConcepts: concepts.filter((c) => !collapsedSet.has(moduleOf.get(c.id))),
    moduleNodes, plainEdges,
    aggregates: [...groups.values()].map((g) => ({ ...g, sum: round2(g.sum) })),
    moduleOf, modules, members, stats,
  };
}

// "Grouped by module" layout: each module gets its own area on a grid, with
// its concepts on a circle inside it, so modules read as separate sub-models
// and their folded blocks never overlap.
function layoutByModule(concepts) {
  const groups = new Map();
  concepts.forEach((c) => {
  ...
  const keys = [...groups.keys()].sort((a, b) => (a === UNCATEGORIZED_MODULE) - (b === UNCATEGORIZED_MODULE) || a.localeCompare(b));
  const radius = (n) => (n <= 1 ? 0 : Math.max(120, (n * 200) / (2 * Math.PI)));
  const maxR = Math.max(0, ...keys.map((k) => radius(groups.get(k).length)));
  const cell = 2 * maxR + 380;
  ...
  keys.forEach((k, gi) => {
    ...
    ms.forEach((c, i) => {
      ...
      pos.set(c.id, { x: Math.round(cx + r * Math.cos(a) - 80), y: Math.round(cy + r * Math.sin(a) - 35) });
    });
  });
  return concepts.map((c) => ({ ...c, position: pos.get(c.id) || c.position }));
}

// A folded module: one block standing for the whole sub-model.
...
// The outline around an unfolded module while other modules are folded, so
// it reads as one sub-model inside the nexus. It never catches clicks except
// on its header, so the map underneath works as normal.
function ModuleFrameNode({ data }) {
  const st = categoryStyle(data.categoryValue);
  return (
    <div
...
// Colour of a bundled link: green when every relationship in it increases
// its target, orange when every one decreases it, violet when mixed.
const aggregateColor = (a) => (a.negative === 0 ? "#16a34a" : a.positive === 0 ? "#ea580c" : "#7c3aed");

const nodeTypes = { concept: ConceptNode, module: ModuleNode, moduleFrame: ModuleFrameNode };
    ...
    <ReactFlowProvider>
      <div className="space-y-5">
        ...
        <ModuleGuide
          ...
          collapsedModules={props.collapsedModules} setCollapsedModules={props.setCollapsedModules}
        />
      </div>
    </ReactFlowProvider>
  );
...
function NetworkTabInner({
  ...
  collapsedModules = EMPTY_ARRAY, setCollapsedModules, translateConcepts,
}) {
  ...
  // ==== folding ==================================================================
  // Only modules present in the current view can be folded; folds of modules
  // outside the scope are simply ignored until they are back in view.
  const presentModuleKeys = useMemo(() => new Set(concepts.map((c) => moduleKey(c.category))), [concepts]);
  // With fewer than two modules in view there is nothing to fold against, so
  ...
  // together than the concepts they stand for, so the overview fills the
  // canvas instead of showing small blocks far apart. Their centres are
  // scaled towards the origin; dragging a block moves its concepts by the
  // unscaled amount. The scale is set when the overview opens and kept while
  // it stays open, so moving a block never makes the others jump.
  const allFolded = folded.moduleNodes.length > 1 && folded.moduleNodes.length === folded.modules.length;
  const overviewScaleRef = useRef(1);
  const overviewScale = useMemo(() => {
    if (!allFolded) return 1;
    const ms = folded.moduleNodes;
    let minD = Infinity;
    for (let i = 0; i < ms.length; i++) for (let j = i + 1; j < ms.length; j++) {
      minD = Math.min(minD, Math.hypot(ms[i].position.x - ms[j].position.x, ms[i].position.y - ms[j].position.y));
    }
    return Math.min(1, Math.max(0.2, (2 * MODULE_NODE_W) / Math.max(1, minD)));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [allFolded, collapsedKey, concepts.length]);
  overviewScaleRef.current = overviewScale;
  const moduleBlocks = useMemo(() => (overviewScale === 1 ? folded.moduleNodes : folded.moduleNodes.map((m) => {
    const cx = (m.position.x + MODULE_NODE_W / 2) * overviewScale, cy = (m.position.y + MODULE_NODE_H / 2) * overviewScale;
    return { ...m, position: { x: cx - MODULE_NODE_W / 2, y: cy - MODULE_NODE_H / 2 } };
  })), [folded.moduleNodes, overviewScale]);
  const moduleNodesRef = useRef(new Map());
  moduleNodesRef.current = new Map(moduleBlocks.map((m) => [m.id, m]));
  const expandModule = useCallback((key) => setCollapsedModules?.((prev) => prev.filter((k) => k !== key)), [setCollapsedModules]);
  const collapseModule = useCallback((key) => setCollapsedModules?.((prev) => (prev.includes(key) ? prev : [...prev, key])), [setCollapsedModules]);
  const openModuleAsModel = useCallback((key) => {
    setCollapsedModules?.((prev) => prev.filter((k) => k !== key));
    setNetworkView?.((v) => ({ ...v, mode: "modules", selected: [key] }));
  }, [setCollapsedModules, setNetworkView]);
  const moduleKeysInView = folded.modules.map((m) => m.key);
  const expandedKeys = moduleKeysInView.filter((k) => !collapsedSet.has(k));
  const structure = collapsedSet.size === 0 ? "full"
    : expandedKeys.length === 0 ? "overview"
    : expandedKeys.length === 1 ? `m:${expandedKeys[0]}` : "custom";
  const showStructure = (value) => {
    if (!setCollapsedModules) return;
    if (value === "full") setCollapsedModules([]);
    else if (value === "overview") setCollapsedModules(moduleKeysInView);
    else if (value.startsWith("m:")) setCollapsedModules(moduleKeysInView.filter((k) => k !== value.slice(2)));
  };

  // Outlines around the unfolded modules while anything is folded, so an
  // opened module reads as one sub-model inside the rest of the nexus.
  const frames = useMemo(() => {
    ...
    const info = new Map(folded.modules.map((m) => [m.key, m]));
    const PAD = 26, HEAD = 30;
    return [...byModule.entries()].map(([k, cs]) => {
      let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
      cs.forEach((c) => {
        const pos = c.position || { x: 0, y: 0 };
        const m = rf.getInternalNode?.(c.id)?.measured;
        minX = Math.min(minX, pos.x); minY = Math.min(minY, pos.y);
        maxX = Math.max(maxX, pos.x + (m?.width || 170)); maxY = Math.max(maxY, pos.y + (m?.height || 64));
      });
      return {
        key: k, id: `frame:${k}`, label: info.get(k)?.label || k, categoryValue: info.get(k)?.categoryValue,
        x: minX - PAD, y: minY - PAD - HEAD, w: maxX - minX + 2 * PAD, h: maxY - minY + 2 * PAD + HEAD,
        memberIds: cs.map((c) => c.id),
      };
    });
  }, [collapsedSet, displayConcepts, folded.modules, rf]);
  const framesRef = useRef(frames);
  framesRef.current = frames;

  // What the canvas draws: unfolded relationships as they are, plus one
  // bundled link per pair of endpoints that involves a folded module.
  ...
  const slotNodes = useMemo(() => [
    ...displayConcepts,
    ...moduleBlocks.map((m) => ({ id: m.id, position: { x: m.position.x + (MODULE_NODE_W - ANCHOR_NODE_W) / 2, y: m.position.y + (MODULE_NODE_H - ANCHOR_NODE_H) / 2 } })),
  ], [displayConcepts, moduleBlocks]);
  const blocksOverlap = useMemo(() => {
    const ms = moduleBlocks;
    for (let i = 0; i < ms.length; i++) for (let j = i + 1; j < ms.length; j++) {
      if (Math.abs(ms[i].position.x - ms[j].position.x) < MODULE_NODE_W && Math.abs(ms[i].position.y - ms[j].position.y) < MODULE_NODE_H) return true;
    }
    return false;
  }, [moduleBlocks]);

  const conceptRfNodes = useMemo(() => displayConcepts.map((c) => ({
  ...
  const rfNodes = useMemo(() => [
    ...frames.map((f) => ({
      id: f.id, type: "moduleFrame", position: { x: f.x, y: f.y },
      data: { ...f, faded: !!fadedNodeIds && f.memberIds.every((id) => fadedNodeIds.has(id)), onCollapse: () => collapseModule(f.key), onOpen: () => openModuleAsModel(f.key) },
      selectable: false, draggable: false, focusable: false, deletable: false, connectable: false,
      zIndex: -1, style: { pointerEvents: "none" },
    })),
    ...conceptRfNodes,
    ...moduleBlocks.map((m) => ({
  ...
  const edgeSlots = useMemo(() => {
    ...
    edgeSlotsRef.current = computeEdgeSlots(slotNodes, displayEdges);
    return edgeSlotsRef.current;
  }, [slotNodes, displayEdges, isDragging]);
  ...
  const onNodesChange = useCallback((changes) => {
    changes.forEach((ch) => {
      if (ch.type === "position" && ch.position) {
        ...
        if (isModuleNodeId(ch.id)) {
          // Dragging a folded module moves every concept inside it by the same
          // amount, so the module keeps its internal layout.
          const mn = moduleNodesRef.current.get(ch.id);
          if (mn && translateConcepts) {
            const k = overviewScaleRef.current || 1;
            const dx = (ch.position.x - mn.position.x) / k, dy = (ch.position.y - mn.position.y) / k;
            if (dx || dy) translateConcepts(mn.memberIds, dx, dy);
          }
        } else if (!isFrameNodeId(ch.id)) {
        ...
        if (isFrameNodeId(ch.id)) return;
        const kind = isModuleNodeId(ch.id) ? "module" : "concept";
        const id = kind === "module" ? ch.id.slice(MODULE_PREFIX.length) : ch.id;
        if (ch.selected) setSelected({ kind, id });
        else setSelected((s) => (s?.kind === kind && s.id === id ? null : s));
      } else if (ch.type === "remove") {
        if (!isModuleNodeId(ch.id) && !isFrameNodeId(ch.id)) removeConcept(ch.id);
      }
    });
  }, [moveConcept, commitHistory, setSelected, removeConcept, translateConcepts]);

  const onEdgesChange = useCallback((changes) => {
    changes.forEach((ch) => {
      const kind = String(ch.id).startsWith("agg:") ? "aggregate" : "edge";
      if (ch.type === "select") {
        if (ch.selected) setSelected({ kind, id: ch.id });
        else setSelected((s) => (s?.kind === kind && s.id === ch.id ? null : s));
      } else if (ch.type === "remove" && kind === "edge") {
        removeEdge(ch.id);
      }
    });
  ...
  const onConnect = useCallback((params) => {
    if (isModuleNodeId(params.source) || isModuleNodeId(params.target)) return;
    commitHistory();
  ...
  const addVisibleConcept = useCallback((pos, category) => {
    commitHistory();
    addConceptAt(pos, category ?? newConceptCategory);
  ...
  const onPaneDoubleClick = useCallback((e) => {
    ...
    // Double-clicking inside an unfolded module's outline adds the concept to that module.
    const frame = framesRef.current.find((f) => pos.x >= f.x && pos.x <= f.x + f.w && pos.y >= f.y && pos.y <= f.y + f.h);
    addVisibleConcept(pos, frame ? frame.categoryValue : undefined);
  ...
  useEffect(() => {
    if (!selected) return;
    let visible = false;
    if (selected.kind === "concept") visible = displayConcepts.some((c) => c.id === selected.id);
    else if (selected.kind === "edge") visible = folded.plainEdges.some((e) => e.id === selected.id);
    else if (selected.kind === "module") visible = collapsedSet.has(selected.id);
    else if (selected.kind === "aggregate") visible = folded.aggregates.some((a) => a.id === selected.id);
    if (!visible) setSelected(null);
  }, [selected, displayConcepts, folded, collapsedSet, setSelected]);

  // Re-frame the map when modules are folded or unfolded.
  const firstFoldRef = useRef(true);
  useEffect(() => {
    if (firstFoldRef.current) { firstFoldRef.current = false; return; }
    const t = setTimeout(() => rf.fitView({ padding: 0.2, duration: 300, maxZoom: FIT_MAX_ZOOM }), 80);
    const t2 = setTimeout(healUnmeasuredNodes, 400);
    return () => { clearTimeout(t); clearTimeout(t2); };
  }, [collapsedKey, rf, healUnmeasuredNodes]);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e) => {
      ...
      if ((e.key === "Delete" || e.key === "Backspace") && (selected?.kind === "concept" || selected?.kind === "edge")) {
        e.preventDefault();
        commitHistory();
  ...
  const runLayout = async (kind) => {
    ...
    try {
      ...
      else if (kind === "modules") next = layoutByModule(concepts);
      else return;
  ...
  const selectedConcept = selected?.kind === "concept" ? displayConcepts.find((c) => c.id === selected.id) : null;
    ...
    <div className="grid grid-cols-[1fr_300px] gap-5 h-[700px]">
      <div className="bg-white rounded-lg border border-slate-200 p-3 flex flex-col relative">
          ...
          <div className="relative">
            ...
            {layoutMenuOpen && (
              <div className="absolute z-30 mt-1 w-56 bg-white border border-slate-200 rounded-md shadow-lg py-1">
                ...
                <button onClick={() => runLayout("modules")} className="w-full text-left px-3 py-1.5 text-xs hover:bg-slate-50 flex items-center gap-2" title="Give every module its own area, so modules read as separate sub-models"><Layers size={13} />Grouped by module</button>
              </div>
            )}
          </div>
        ...
        {collapsedSet.size > 0 && (
          <p className="mb-2 text-[11px] text-slate-600 bg-slate-50 border border-slate-200 rounded px-2 py-1">
            ...
            {" "}Folding only changes the map; analyses follow the analysis scope.
            {blocksOverlap && <span className="text-amber-700"> Some blocks overlap because their concepts are spread across the map: Auto Arrange &rarr; Grouped by module gives each module its own area.</span>}
          </p>
        )}
        ...
        >
          ...
          <ErrorBoundary label="the network canvas">
            ...
            >
              ...
              <MiniMap pannable zoomable nodeColor={(n) => (n.type === "moduleFrame" ? "transparent" : n.type === "module" ? categoryStyle(n.data?.categoryValue).border : categoryStyle(n.data?.concept?.category).bg)} className="!bg-white" />
            </ReactFlow>
          </ErrorBoundary>
        </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-4 overflow-auto flex flex-col">
                  ...
                  <div className="mt-2 rounded-md border border-slate-200 bg-slate-50 p-2 space-y-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        onClick={() => {
                          commitHistory();
                          const otherModule = moduleKey(conceptById.get(linkTarget)?.category);
                          if (collapsedSet.has(otherModule)) expandModule(otherModule);
        ...
        {selected?.kind === "module" && (() => {
          ...
          const m = folded.modules.find((x) => x.key === key);
          const st = folded.stats.get(key) || { internal: 0, out: 0, in: 0, partners: new Map() };
          const members = folded.members.get(key) || [];
          const labelOf = new Map(folded.modules.map((x) => [x.key, x.label]));
          const sty = categoryStyle(m?.categoryValue);
          return (
        ...
        {selected?.kind === "aggregate" && (() => {
          const a = folded.aggregates.find((x) => x.id === selected.id);
          if (!a) return null;
          const labelOfModule = new Map(folded.modules.map((x) => [x.key, x.label]));
          const endLabel = (id) => (isModuleNodeId(id) ? `${labelOfModule.get(id.slice(MODULE_PREFIX.length))} (module)` : conceptById.get(id)?.name);
              ...
              <div>
                ...
                {under.map((e) => (
                  <button
                    key={e.id}
                    onClick={() => {
                      const ks = [moduleKey(conceptById.get(e.source)?.category), moduleKey(conceptById.get(e.target)?.category)];
                      setCollapsedModules?.((prev) => prev.filter((k) => !ks.includes(k)));
                      setSelected({ kind: "edge", id: e.id });
                    }}
                    className="w-full flex items-center gap-1.5 text-left px-1.5 py-1 rounded hover:bg-slate-100"
                    title="Unfold the modules involved and select this relationship"
                  >
                    <span className="flex-1 truncate">{conceptById.get(e.source)?.name} &rarr; {conceptById.get(e.target)?.name}</span>
                    <span className={`font-mono shrink-0 ${e.weight >= 0 ? "text-green-700" : "text-orange-700"}`}>{e.weight > 0 ? "+" : ""}{round2(e.weight)}</span>
                  </button>
                ))}
              </div>
...
function ModuleGuide({ concepts, edges, networkView, setNetworkView, viewInfo, displayMode, collapsedModules = EMPTY_ARRAY, setCollapsedModules }) {
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 text-xs text-slate-600 leading-relaxed">
            ...
            <div className="space-y-1.5">
              ...
              <p>
                <strong>Folding</strong> turns a module into a single block, so it reads as a sub-model of the nexus. <strong>Full model</strong> shows every concept,
                <strong> Module overview</strong> folds every module and shows how the modules are linked, and <strong>Module view</strong> opens one module with the others
                folded around it. Folding only changes the map, not the analyses.
              </p>
...
function downloadCSV(rows, filename) {
  // The UTF-8 byte-order mark makes Excel read accented names correctly.
...
async function buildAnalysisWorkbook({
  ...
  addDataSheet(workbook, "Model Structure", [
    ...
    { header: "Module", key: "category" }, { header: "Definition", key: "description" },
...
// guard; this catches anything outside them (the header, the scope bar, or a
// problem while starting up), so the page can never end up blank with no way
// forward. It offers a reload, a download of the model saved in this browser,
// and, as a last step, starting fresh.
class AppCrashBoundary extends React.Component {
  constructor(props) {
    super(props);
    this.state = { error: null, confirmFresh: false };
  }
  static getDerivedStateFromError(error) { return { error }; }
  componentDidCatch(error, info) { console.error("[The Spaghetti Engine] unrecoverable render error:", error, info?.componentStack); }
  downloadSaved() {
    try {
      const raw = localStorage.getItem(AUTOSAVE_KEY);
      if (!raw) { alert("There is no model saved in this browser."); return; }
      const a = document.createElement("a");
      a.href = URL.createObjectURL(new Blob([raw], { type: "application/json" }));
      a.download = "spaghetti-engine-saved-model.json";
      a.click();
    } catch { alert("The saved model could not be read."); }
  }
  render() {
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            {!this.state.confirmFresh ? (
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-md bg-rose-700 text-white text-sm"
              >Yes, remove the saved model and start fresh</button>
            )}
          </div>
        </div>
      </div>
...
function SpaghettiEngineApp() {
  ...
  const [edges, setEdges] = useState(() => restored?.edges ?? makeTemplate("nexus").edges);
  const [restoreNotice, setRestoreNotice] = useState(() => !!restored?.concepts?.length);
  // What the import cleaner had to correct in the last imported file, if anything.
  const [importNotice, setImportNotice] = useState(() => (restored?.fixes?.length ? { file: "the model saved in this browser", fixes: restored.fixes } : null));
  ...
  // Modules folded into a single block on the Network tab (display only).
  ...
  // ==== confirmation dialog ======================================================
  // An in-app dialog rather than the browser's window.confirm: some browser
  // environments (embedded browsers, or a page whose dialogs the user once
  // blocked) suppress native dialogs and make window.confirm return false
  // without showing anything, which silently cancelled every model
  // replacement. askConfirm resolves true / false once the user chooses.
  const [confirmState, setConfirmState] = useState(null);
  const askConfirm = useCallback((opts) => new Promise((resolve) => setConfirmState({ ...opts, resolve })), []);
  const closeConfirm = (value) => { setConfirmState((st) => { st?.resolve(value); return null; }); };
  ...
  // Moves a set of concepts by the same offset (dragging a folded module).
  const translateConcepts = useCallback((ids, dx, dy) => {
    const set = new Set(ids);
    setConcepts((cs) => cs.map((c) => (set.has(c.id) ? { ...c, position: { x: (c.position?.x ?? 0) + dx, y: (c.position?.y ?? 0) + dy } } : c)));
  }, []);
  ...
  const toggleCompare = (id) => {
    if (id === "baseline") return; // the baseline is always part of the comparison
    setCompareIds((ids) => (ids.includes(id) ? ids.filter((i) => i !== id) : [...ids, id]));
  };
  // The scenarios shown against the baseline. The baseline itself is always
  // included separately, exactly once, so it is never one of these (older
  // saved states could still carry it in compareIds; it is filtered out here
  // rather than drawn a second time), and ids of deleted scenarios are ignored.
  const comparedIds = useMemo(
    () => compareIds.filter((id) => id !== "baseline" && scenarios.some((s) => s.id === id)),
    [compareIds, scenarios]
  );
  // Display names for the comparison: the baseline is always labelled
  // "Baseline", whatever its scenario row is called, and scenarios that share a
  // name get a number so no two series are indistinguishable in the legend.
  const comparisonNames = useMemo(() => {
    const names = { baseline: "Baseline" };
    const used = new Map([["baseline", 1]]);
    comparedIds.forEach((id) => {
      const base = (scenarios.find((s) => s.id === id)?.name || "").trim() || "Unnamed scenario";
      const key = base.toLowerCase();
      const n = (used.get(key) || 0) + 1;
      used.set(key, n);
      names[id] = n > 1 ? `${base} (${n})` : base;
    });
    return names;
  }, [comparedIds, scenarios]);
  const notYetRun = comparedIds.filter((id) => !results[id]);
  ...
  const confirmReplace = async (what) => {
    if (!concepts.length) return true;
    return askConfirm({
      title: "Replace the current model?",
      message: `${what} replaces your current model (${concepts.length} concept${concepts.length === 1 ? "" : "s"}, ${edges.length} relationship${edges.length === 1 ? "" : "s"}). Its concepts and relationships can be brought back with Undo (Ctrl+Z) until you reload the page.`,
      confirmLabel: "Replace model",
      secondary: { label: "Download a copy first, then replace", action: exportModel },
    });
  };
  ...
  const importModel = (e) => {
    ...
    reader.onload = async () => {
      try {
        const data = sanitizeModel(JSON.parse(reader.result));
        if (!data) throw new Error("not a model");
        if (!(await confirmReplace(`Importing "${file.name}"`))) return;
        commitHistory();
        ...
        setCompareIds([]);
        setSelected(null);
        setImportNotice(data.fixes.length ? { file: file.name, fixes: data.fixes } : null);
  ...
  const exportComparisonCSV = () => {
    const ids = ["baseline", ...comparedIds];
    const header = ["Concept", ...ids.map((id) => comparisonNames[id] || id)];
  ...
  // ==== model tables (Model editor) ============================================
  // The model itself as tables: concepts with their definitions, the list of
  // relationships, and the adjacency matrix. Always the complete model,
  // whatever the analysis scope. The matrix uses the same layout the
  // "Build from an adjacency matrix" panel reads, so it can be pasted back.
  const modelTables = () => {
    const nameOf = new Map(concepts.map((c) => [c.id, c.name]));
    const labels = new Map(listModules(concepts).map((m) => [m.key, m.key === UNCATEGORIZED_MODULE ? "" : m.label]));
    const moduleOf = (c) => labels.get(moduleKey(c?.category)) ?? "";
    const byId = new Map(concepts.map((c) => [c.id, c]));
    const conceptRows = concepts.map((c) => ({
      id: c.id, name: c.name, module: moduleOf(c), definition: c.description || "", start: c.initialValue ?? 0,
      out: edges.filter((e) => e.source === c.id).length, in: edges.filter((e) => e.target === c.id).length,
    }));
    const relationshipRows = edges.map((e) => ({
      from: nameOf.get(e.source), to: nameOf.get(e.target), weight: e.weight,
      effect: e.weight > 0 ? "increases" : e.weight < 0 ? "decreases" : "no effect",
      strength: strengthWord(e.weight).replace(/^an? /, ""),
      sentence: relationshipSentence(nameOf.get(e.source), nameOf.get(e.target), e.weight),
      fromModule: moduleOf(byId.get(e.source)), toModule: moduleOf(byId.get(e.target)),
      between: moduleKey(byId.get(e.source)?.category) !== moduleKey(byId.get(e.target)?.category) ? "yes" : "no",
      description: e.description || "", fromId: e.source, toId: e.target,
    }));
    const weight = new Map(edges.map((e) => [`${e.source}>${e.target}`, e.weight]));
    const matrix = concepts.map((r) => [r.name, ...concepts.map((col) => weight.get(`${r.id}>${col.id}`) ?? 0)]);
    return { conceptRows, relationshipRows, matrixHeader: ["", ...concepts.map((c) => c.name)], matrix };
  };
  const CONCEPT_COLUMNS = [["id", "ID"], ["name", "Concept"], ["module", "Module"], ["definition", "Definition"], ["start", "Starting value"], ["out", "Relationships out"], ["in", "Relationships in"]];
  const RELATIONSHIP_COLUMNS = [["from", "From"], ["to", "To"], ["weight", "Weight"], ["effect", "Effect"], ["strength", "Strength"], ["sentence", "In words"], ["fromModule", "From module"], ["toModule", "To module"], ["between", "Between modules"], ["description", "Description"], ["fromId", "From ID"], ["toId", "To ID"]];
  const exportModelTable = (kind) => {
    const t = modelTables();
    if (kind === "concepts") downloadCSV([CONCEPT_COLUMNS.map((c) => c[1]), ...t.conceptRows.map((r) => CONCEPT_COLUMNS.map((c) => r[c[0]]))], "spaghetti-engine-concepts.csv");
    if (kind === "relationships") downloadCSV([RELATIONSHIP_COLUMNS.map((c) => c[1]), ...t.relationshipRows.map((r) => RELATIONSHIP_COLUMNS.map((c) => r[c[0]]))], "spaghetti-engine-relationships.csv");
    if (kind === "matrix") downloadCSV([t.matrixHeader, ...t.matrix], "spaghetti-engine-adjacency-matrix.csv");
  };
  const exportModelTablesXlsx = async () => {
    const t = modelTables();
    const workbook = new ExcelJS.Workbook();
    workbook.creator = "The Spaghetti Engine";
    const used = new Set();
    addDataSheet(workbook, "Concepts", CONCEPT_COLUMNS.map(([key, header]) => ({ key, header })), t.conceptRows, used);
    addDataSheet(workbook, "Relationships", RELATIONSHIP_COLUMNS.map(([key, header]) => ({ key, header })), t.relationshipRows, used);
    const cols = [{ key: "__row", header: "From \\ To" }, ...concepts.map((c, i) => ({ key: `c${i}`, header: c.name }))];
    addDataSheet(workbook, "Adjacency Matrix", cols, t.matrix.map((r) => Object.fromEntries([["__row", r[0]], ...r.slice(1).map((v, i) => [`c${i}`, v])])), used);
    ...
    a.download = "spaghetti-engine-model-tables.xlsx";
    a.click();
  };

  const buildFromMatrix = async () => {
    setMatrixError(null);
    ...
    try {
      ...
      if (!(await confirmReplace("Building from the pasted matrix"))) return;
      commitHistory();
      setRestoreNotice(false);
  ...
  const loadTemplate = async (kind, { skipConfirm = false } = {}) => {
    const label = kind === "blank" ? "Starting a blank model" : "Loading this example";
    if (!skipConfirm && !(await confirmReplace(label))) return;
    commitHistory();
    setRestoreNotice(false);
    setSelected(null);
  ...
  const componentChangeData = useMemo(() => {
    if (comparedIds.length !== 1) return null;
    const base = results.baseline;
    const scen = results[comparedIds[0]];
  ...
  const highlightConcept = useCallback((id) => {
    const c = latestModelRef.current.concepts.find((x) => x.id === id);
    if (c) { const k = moduleKey(c.category); setCollapsedModules((prev) => prev.filter((x) => x !== k)); }
    setSelected({ kind: "concept", id });
  }, []);
  ...
  const revealItem = (kind, id) => {
    ...
    // Unfold whichever module(s) the item sits in, so it is on the map to select.
    const moduleOfId = (cid) => moduleKey(concepts.find((c) => c.id === cid)?.category);
    const unfold = kind === "edge" && e ? [moduleOfId(e.source), moduleOfId(e.target)] : kind === "concept" ? [moduleOfId(id)] : [];
    if (unfold.length) setCollapsedModules((prev) => prev.filter((k) => !unfold.includes(k)));
    setSelected({ kind, id });
    setTab("network");
  ...
  return (
    <div className="w-full h-full min-h-[820px] bg-white text-slate-800 flex flex-col" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      {restoreNotice && (
        <div className="flex items-center gap-3 flex-wrap px-5 py-2 text-xs bg-teal-50 border-b border-teal-200 text-teal-900">
          ...
          <span className="ml-auto flex items-center gap-3">
            ...
            <button onClick={() => loadTemplate("nexus", { skipConfirm: true })} className="underline hover:no-underline">Open the example model</button>
          </span>
        </div>
      )}

      {confirmState && (
        <div className="fixed inset-0 z-[60] bg-slate-900/40 flex items-center justify-center p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) closeConfirm(false); }}>
          <div
            role="dialog" aria-modal="true" aria-labelledby="se-confirm-title"
      ...
      {importNotice && (
        <div className="flex items-start gap-3 px-5 py-2 text-xs bg-amber-50 border-b border-amber-200 text-amber-900">
          <Info size={14} className="text-amber-700 shrink-0 mt-0.5" />
          <div className="flex-1">
            <strong>{importNotice.file === "the model saved in this browser" ? "The saved model needed some repairs." : `"${importNotice.file}" was imported with some corrections.`}</strong>
            <ul className="list-disc pl-4 mt-0.5 space-y-0.5">{importNotice.fixes.map((f, i) => <li key={i}>{f}</li>)}</ul>
          </div>
          <button onClick={() => setImportNotice(null)} className="underline hover:no-underline shrink-0">Dismiss</button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-auto p-5 bg-slate-50">
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-4">
              ...
              <div className="flex items-center gap-2 flex-wrap mt-3">
                <Btn variant="accent" onClick={exportModelTablesXlsx} disabled={!concepts.length} title="One Excel file with three sheets: Concepts, Relationships, Adjacency Matrix">
                  <Table2 size={14} />All three as Excel (.xlsx)
                </Btn>
                <Btn variant="outline" onClick={() => exportModelTable("concepts")} disabled={!concepts.length}><Download size={13} />Concepts with definitions (CSV)</Btn>
        ...
        {tab === "network" && (
          <NetworkTab
            ...
            collapsedModules={collapsedModules} setCollapsedModules={setCollapsedModules} translateConcepts={translateConcepts}
            onReveal={revealItem}
          />
        )}
                ...
                <div className="bg-white rounded-lg border border-slate-200 p-4" data-chart="scenario-comparison">
                  ...
                  {!Object.keys(results).length ? (
                    ...
                    <>
                      ...
                      {notYetRun.length > 0 && (
                        <div className="mb-3 rounded-md border border-amber-200 bg-amber-50 p-2.5 text-xs text-amber-800 flex items-start gap-1.5">
                          <Info size={13} className="shrink-0 mt-0.5" />
                          <span>Not run yet: {notYetRun.map((id) => `"${comparisonNames[id]}"`).join(", ")}. Click Run simulation to include {notYetRun.length === 1 ? "it" : "them"}.</span>
                        </div>
                      )}
                      <div className="mb-4">
                        <MethodPanel
                          ...
                          stats={[
                            ...
                            { label: "Update rule / mode", value: `${settings.updateRule ?? "relative"} / ${settings.mode ?? "synchronous"}` },
                            {
                              ...
                              value: ["baseline", ...comparedIds].every((id) => !results[id] || results[id].converged) ? "Yes" : "No",
                      ...
                      {(() => {
                        // Rows keyed by scenario id (never by name, which can be
                        // edited or shared by two scenarios); names are only for
                        // display. A scenario that has not been run yet has no
                        // value (null: no bar), rather than a misleading 0.
                        const seriesIds = ["baseline", ...comparedIds];
                        const data = aConcepts.map((c) => {
                          const row = { concept: c.name };
                          seriesIds.forEach((id) => { row[id] = results[id] ? round2(results[id].final[c.id] ?? 0) : null; });
                          return row;
                        });
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="text-left text-slate-400 border-b border-slate-100">
                            ...
                            {comparedIds.length === 1 && <th className="py-1 font-medium">Δ</th>}
                          </tr>
                        </thead>
                        <tbody>
                          {aConcepts.map((c) => {
                              ...
                              <tr key={c.id} className="border-b border-slate-50">
                                ...
                                {comparedIds.length === 1 && (
                                  <td className="py-1 font-mono">
                                    {b !== undefined && results[comparedIds[0]]?.final?.[c.id] !== undefined
                                      ? (results[comparedIds[0]].final[c.id] - b >= 0 ? "+" : "") + round2(results[comparedIds[0]].final[c.id] - b).toFixed(2)
                                      : "n/a"}
                                  </td>
                                )}
                              </tr>
